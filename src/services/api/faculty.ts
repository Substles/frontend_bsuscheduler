...
export async function updateFaculty(id: number, name: string, initials: string, password: string) {
  const response = await api.put("/faculties/update", { id, name, initials, password });
  return response;
}
